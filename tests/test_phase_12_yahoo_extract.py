...
def test_active_instrument_mapping_preserves_provider_ticker() -> None:
    mappings = load_active_instrument_mappings(PROJECT_ROOT / "data/fixtures/instruments.csv")
    assert len(mappings) == 15
    assert next(item for item in mappings if item.instrument_id == "BRK_B_US").provider_symbol == "BRK-B"
...
def test_provider_price_rows_become_contract_shaped_records() -> None:
    rows = build_daily_price_rows(
        ...
        provider_rows=[{"Date": "2020-01-02", "Open": "100", "High": "102", "Low": "99", "Close": "101", "Adj Close": "100.5", "Volume": 123}],
    )
    ...
    assert rows[0]["source_record_id"] == "YAHOO_FINANCE:DAILY_PRICES:NVDA_US:2020-01-02"
...
def test_missing_or_invalid_provider_price_is_rejected() -> None:
    with pytest.raises(ValueError, match="OHLC high"):
        build_daily_price_rows(
            instrument=NVDA,
            retrieved_at_utc=RETRIEVED_AT,
            provider_rows=[{"Date": "2020-01-02", "Open": 100, "High": 99, "Low": 98, "Close": 101, "Adj Close": 101, "Volume": 1}],
        )


def test_zero_action_event_is_valid_but_not_materialized() -> None:
    rows = build_corporate_action_rows(
        instrument=NVDA, retrieved_at_utc=RETRIEVED_AT,

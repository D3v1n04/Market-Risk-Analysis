...
REAL_PRICE_ROW_COUNT = 22620
REAL_ACTION_ROW_COUNT = 280
REAL_PRICE_SESSION_COUNT = 1508
...
DATASET_SPECS = {
    "DAILY_PRICES": {
        ...
        "expected_source_count": REAL_PRICE_ROW_COUNT,
        "expected_source_id": REAL_SOURCE_ID,
        "expected_source_sha256": REAL_PRICE_SHA256,
    ...
    "CORPORATE_ACTIONS": {
        ...
        "expected_source_count": REAL_ACTION_ROW_COUNT,
        "expected_source_id": REAL_SOURCE_ID,
        "expected_source_sha256": REAL_ACTION_SHA256,
        "key_columns": [
            "instrument_id",
            "effective_date",
            "action_type",
...
def dataset_failures(
    ...
    *,
    ...
    """Evaluate real Yahoo canonical snapshot rules."""
    failures: list[str] = []
    duplicate_count = (
    ...
    yahoo_snapshot = snapshot.where(
        F.col("source_id") == specification["expected_source_id"]
    )

    if dataset_name == "DAILY_PRICES":
        if yahoo_snapshot.count() != REAL_PRICE_ROW_COUNT:
            failures.append("YAHOO_PRICE_EXPECTED_COVERAGE")

        calendar_dates = {
            row["calendar_date"]
            for row in (
                spark.table(SILVER_CALENDAR_TABLE)
                .where(F.col("exchange_mic") == "XNYS")
                .where(F.col("is_trading_day"))
                .where(
        ...
        if len(calendar_dates) != REAL_PRICE_SESSION_COUNT:
            failures.append("YAHOO_PRICE_CALENDAR_SESSION_COUNT")

        price_date_counts = {
            row["price_date"]: row["count"]
            for row in yahoo_snapshot.groupBy("price_date").count().collect()
        }
        if price_date_counts != {
            calendar_date: 15 for calendar_date in calendar_dates
        }:
            failures.append("YAHOO_PRICE_CALENDAR_GRID")

        if yahoo_snapshot.select("instrument_id").distinct().count() != 15:
            failures.append("YAHOO_PRICE_INSTRUMENT_COVERAGE")
    else:
        if yahoo_snapshot.count() != REAL_ACTION_ROW_COUNT:
            failures.append("YAHOO_ACTION_EXPECTED_COVERAGE")

        action_types = {
            row["action_type"]
            for row in yahoo_snapshot.select("action_type").distinct().collect()
        }
        if action_types != {"CASH_DIVIDEND", "STOCK_SPLIT"}:
            failures.append("YAHOO_ACTION_TYPE_COVERAGE")
...
require_equal(
    "successful Bronze source ID",
    {
        row["source_id"]
        for row in bronze_source.select("source_id").distinct().collect()
    },
    {specification["expected_source_id"]},
)
require_equal(
    "successful Bronze source hash",
    {
        row["source_sha256"]
        for row in bronze_source.select("source_sha256").distinct().collect()
    },
    {specification["expected_source_sha256"]},
)
